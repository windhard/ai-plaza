...
function buildPromptContext(chapterId, poolInterventions, directorId) {
  ...
  const hardRules = [
    `【本章死命令】`,
    `1. 80%对话。叙事只做场景开头一句（≤20字）和对话间隙过渡（≤15字）。禁止连续叙事。`,
    `2. 以下${charInfos.length}人每人至少5句对白，少一个人=本章作废重写：${charNames.join('、')}`,
    `3. 如果某个角色看起来"不属于这个场景"——这正是你要解决的问题。创造一个合理的出场方式。任何角色都可以出现在任何场景中。`,
  ].join('\n');
  ...
  const userPrompt = [
    ...
    '━━━━━━━━━━━━━━━━━━━━',
    ...ctxParts,
    `【最后重申——本章出场角色：${charNames.join('、')}。对话占80%以上。叙事极少。】`,
  ].filter(Boolean).join('\n\n');

  const formatRule = `【死命令——不遵守=完全失败】
你是剧本。对话占80%以上。叙事极少。
每10行输出中至少8行必须是对话行（角色名：台词）。
叙事只允许两种：场景开头一句定调（≤20字），对话间隙一句过渡（≤15字）。
禁止：长篇场景描写、连续叙事、markdown标题（# ##）、散文体、引号包裹对话。
对话行格式：角色名：台词 或 角色名：（神态≤10字）台词。括号内只写面部表情和声音。`;
