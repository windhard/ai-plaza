...
function buildPromptContext(chapterId, poolInterventions, directorId) {
  ...
  const formatRule = `【⚠️ 最高优先级：输出格式】
你的输出是舞台剧本，只有两种元素：
1. 叙事行：场景、动作、氛围。用（括号）或普通文字。角色的身体动作（走、跑、跪、推门、翻开裙子等）都写在这里。
2. 对话行：角色名：台词 或 角色名：（神态语气）台词
——括号里只写该说话者的面部表情和声音质感：抿嘴/眼眶红/嘴角颤/眼神躲闪/声音发抖/嗓子发干/语调平静得不正常。不超过12字。
——禁止在对话括号里写肢体动作（推门/跑过来/跪下/掀裙/递东西）。那是叙事行的事。
——对话行必须以角色名开头，紧跟中文冒号。禁止引号。禁止散文体。`;
...
如果角色是超自然存在（恶魔/触手/地狱生物）而场景是日常空间：
- 它可以以"只有主角能感知"的方式出现：镜中倒影、耳语、影子、幻觉、物体莫名移动
- 它可以在叙事中以"无形的在场"影响氛围：温度骤降、灯光闪烁、主角莫名心悸
- 它可以在关键时刻现身说话——其他角色看不见它，但主角看见了，主角的反应是真实的
- 它必须说话。沉默的超自然存在不算出场。

如果角色是普通人类而场景中没有自然交集：
- 创造合理的交集：偶遇、电话、消息、路过、工作交接
- 哪怕是30秒的走廊偶遇+一句对白，也算出场
- 每个角色的对白必须体现其人格核心，不能是泛泛的"早""你好"`;
