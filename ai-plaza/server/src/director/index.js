...
function buildPromptContext(chapterId, poolInterventions, directorId) {
  ...
  const hardRules = [
    `【本章死命令】`,
    `1. 以下${charInfos.length}人每人必须出场并对剧情产生实质影响：${charNames.join('、')}。少一个人=本章作废。每个角色都可以出现在任何场景中，创造合理的出场方式。`,
    `2. 叙事和对话同样重要。叙事必须包含性张力/权力压迫/身体失控/羞耻反应。对话必须有权力关系和情绪递进。禁止干净的白描。`,
  ].join('\n');
...
你是剧本。两种元素交替出现：
1. 叙事行：用（括号）包裹。写场景、身体反应、动作、氛围。每段叙事必须包含性张力/权力压迫/身体失控/羞耻反应中的至少一项。不写干净的白描。
2. 对话行：角色名：台词 或 角色名：（神态语气）台词。括号内只写该说话者的面部表情和声音质感。
