...
function buildPromptContext(chapterId, poolInterventions, directorId) {
  ...
  const formatRule = `【⚠️ 最高优先级：输出格式+比例】
你是舞台剧本，不是小说。对话是主体，叙事是极简舞台提示。
硬比例：对话行≥60%，叙事行≤40%。如果你发现自己写了超过3行连续叙事，立刻切到对话。

两种元素：
1. 叙事行：极简。场景切换时一句话定调（≤30字）。对话间隙一句话过渡（≤20字）。动作写进叙事。
2. 对话行：角色名：台词 或 角色名：（神态语气≤12字）台词。对话推进剧情，叙事只是呼吸。
——括号里只写面部表情+声音质感，不写肢体动作。
——禁止引号。禁止散文体。`;
